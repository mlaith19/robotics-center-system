...
export const GET = withTenantAuth(async (req, session) => {
  ...
  const teacherIdScope = session.roleKey !== "super_admin" && (session.role || "").toLowerCase() === "teacher"
    ? await getTeacherIdForUser(session.id, db)
    : teacherIdParam
  ...
  try {
    ...
    const countResult = await db.unsafe(`SELECT COUNT(*) as c FROM "Student" s ${whereClause}`, params)
    ...
    const [allCount, activeCount, newInPeriod] = await Promise.all([
      db`SELECT COUNT(*) as c FROM "Student"`,
      db`SELECT COUNT(*) as c FROM "Student" WHERE status IN ('active', 'פעיל') OR status IS NULL`,
      startDate && endDate
        ? db`SELECT COUNT(*) as c FROM "Student" WHERE "createdAt" >= ${startDate} AND "createdAt" <= ${endDate + "T23:59:59.999Z"}`
        : Promise.resolve([{ c: 0 }]),
    ])
