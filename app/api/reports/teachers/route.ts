...
export const GET = withTenantAuth(async (req, session) => {
  ...
  const teacherIdScope = isTeacher ? await getTeacherIdForUser(session.id, db) : null
  ...
  try {
    ...
    const countResult = await db.unsafe(`SELECT COUNT(*) as c FROM "Teacher" t ${whereClause}`, params)
    ...
    const [allCount, activeCount, newInPeriod] = await Promise.all([
      db`SELECT COUNT(*) as c FROM "Teacher"`,
      db`SELECT COUNT(*) as c FROM "Teacher" WHERE status IN ('active', 'פעיל') OR status IS NULL`,
      startDate && endDate
        ? db.unsafe(`SELECT COUNT(*) as c FROM "Teacher" t WHERE t."createdAt" >= $1::timestamp AND t."createdAt" <= $2::timestamp`, [startDate, endDate + "T23:59:59.999Z"])
        : Promise.resolve([{ c: 0 }]),
    ])
