...
export const GET = withTenantAuth(async (req, session) => {
  ...
  try {
    ...
    const countResult = await db.unsafe(`SELECT COUNT(*) as c FROM "Payment" p ${whereClause}`, params)
    const total = Number((countResult[0] as { c: string })?.c ?? 0)

    const sumResult = await db.unsafe(`SELECT COALESCE(SUM(p.amount), 0) as s FROM "Payment" p ${whereClause}`, params)
