...
export default function AttendancePage() {
  ...
  const attendanceSWRKey = useMemo(
    () =>
      ((isCourseBasedAttendance && !!activeCourseId) || (!isCourseBasedAttendance && !!selectedId)) && selectedDate
        ...
        : null,
    [isCourseBasedAttendance, activeCourseId, selectedId, selectedDate, dateStr]
  )
  const { data: existingAttendance = [] } = useSWR(attendanceSWRKey, arrayFetcher)

  // Sync server data into local state only when the query key changes (date/course/student
  // selection changes). Skipping on background revalidations prevents SWR from overwriting
  // optimistic updates that handleStatusChange already applied.
  useEffect(() => {
    const attendanceRecord: Record<string, AttendanceStatus> = {}
    const createdByRecord: Record<string, string> = {}
    ;(existingAttendance as any[]).forEach((a: any) => {
