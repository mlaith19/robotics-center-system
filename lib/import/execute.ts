...
async function findStudentByUniqueKey(
  ...
  if (n) {
    const r = await db`SELECT id FROM "Student" WHERE "idNumber" = ${n} LIMIT 1`
    if (r.length) return (r[0] as { id: string }).id
  }
  if (p) {
    const r = await db`SELECT id FROM "Student" WHERE phone = ${p} OR REPLACE(REPLACE(REPLACE(phone, ' ', ''), '-', ''), '.', '') = ${p} LIMIT 1`
    if (r.length) return (r[0] as { id: string }).id
  }
  if (e) {
    const r = await db`SELECT id FROM "Student" WHERE email = ${e} LIMIT 1`
    if (r.length) return (r[0] as { id: string }).id
  }
  return null
}

async function findTeacherByUniqueKey(db: Sql, phone: string | null, email: string | null): Promise<string | null> {
  const p = normalizePhone(phone ?? "")
  const e = normalizeString(email)
  if (p) {
    const r = await db`SELECT id FROM "Teacher" WHERE phone = ${p} OR REPLACE(REPLACE(REPLACE(COALESCE(phone,''), ' ', ''), '-', ''), '.', '') = ${p} LIMIT 1`
    if (r.length) return (r[0] as { id: string }).id
  }
  if (e) {
    const r = await db`SELECT id FROM "Teacher" WHERE email = ${e} LIMIT 1`
    if (r.length) return (r[0] as { id: string }).id
  }
  return null
}

async function resolveStudentId(db: Sql, identifier: string): Promise<string | null> {
  const trimmed = identifier.trim()
  const idNum = /^\d+$/.test(trimmed) ? trimmed : null
  const em = trimmed.includes("@") ? trimmed : null
  const ph = !em ? normalizePhone(identifier) : ""
  return findStudentByUniqueKey(db, idNum, ph || null, em)
}
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      if (entity === "students") {
        ...
        const existingId = await findStudentByUniqueKey(db, idNumber, phone, email)
        ...
        const phone = normalizeString(mapped.phone) || null
        const email = normalizeString(mapped.email) || null
        const existingId = await findTeacherByUniqueKey(db, phone, email)
        ...
        const studentId = await resolveStudentId(db, studentIdentifier)
        if (!studentId) {
          failed++
